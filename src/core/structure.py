...
class DromPagination(Base):
    ...
    def _build_url(page_no: int, brand: str) -> str:
        return f'{Link.domain.value}{brand}{Link.page_prefix.value}{page_no}/'
...
class Drom:
    ...
    async def check_brand(brand: str) -> bool:
        ...
        async with aiohttp.ClientSession() as session:
            url = f'{Link.domain.value}{brand.lower()}/'
            async with session.get(url) as resp:
                if resp.status == 200:
                    return True
                return False
    ...
    def save_items(items: List[Car], brand: str) -> None:
        ind: int
        with open(f'result/{brand}.csv', 'w', encoding='utf-8') as csvfile:
            csv_writer = csv.writer(csvfile, delimiter=';')
            csv_writer.writerow(['№', 'Model', 'Year', 'Color', 'Price', 'Mileage(km)',
                                 'Engine', 'Horse Power', 'Transmission'])
            ind = 1
            for item in items:
                if item.year + item.price + item.mileage != '':
                    csv_writer.writerow([ind, item.model, item.year, item.color, item.price,
